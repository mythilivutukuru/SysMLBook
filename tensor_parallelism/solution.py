...
def shard_columns(B: np.ndarray, num_gpus: int) -> list[np.ndarray]:
    ...
    """
    # --- SOLUTION ---
    return np.array_split(B, num_gpus, axis=1)
    # --- END SOLUTION ---
...
def tensor_parallel_matmul(
    ...
    # --- SOLUTION ---
    B_shards = shard_columns(B, num_gpus)
    # --- END SOLUTION ---
    ...
    for _ in range(num_gpus):
        rank, C_local = result_queue.get()
        partial_results[rank] = C_local
    # --- END SOLUTION ---
    ...
    # --- SOLUTION ---
    C = np.concatenate(
        [partial_results[r] for r in sorted(partial_results)],
        axis=1,
    )
    # --- END SOLUTION ---
