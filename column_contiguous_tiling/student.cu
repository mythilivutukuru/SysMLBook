...
__global__ void matmul_tiled_col_contiguous_kernel(const float* __restrict__ A,
    ...
    // TODO: Compute blockRow, blockCol from blockIdx.
    // TODO: Compute localRow, localCol from threadIdx.
    //       (0 .. TILE-1 for each)

    // TODO: Compute baseGlobalRow = blockRow * (TILE * elems_per_thread)
    // TODO: Compute globalCol = blockCol * TILE + localCol
    // TODO: Compute threadRowStart = baseGlobalRow + localRow * elems_per_thread
    //       (this thread's first output row; it owns threadRowStart .. threadRowStart + elems_per_thread - 1)

    // TODO: Declare float acc[MAX_ELEMS_PER_THREAD] and zero-initialize
    //       the first elems_per_thread entries.
    ...
    for (int t = 0; t < numTiles; ++t) {
        // --- Load A tile ---
        // TODO: Each thread loads elems_per_thread CONTIGUOUS elements of A.
        //       Column to load: aColBase = t * TILE + localCol
        //       For e in [0, elems_per_thread):
        //         gRow = threadRowStart + e
        //         value = A[gRow * N + aColBase] if (gRow < N && aColBase < N) else 0.0f
        //         store into sA[(localRow * elems_per_thread + e) * sA_row_stride + localCol]

        // --- Load B tile (TILE x TILE) ---
        // TODO: Each thread loads ONE element of B.
        //       bRow = t * TILE + localRow
        //       bColBase = blockCol * TILE
        //       value = B[bRow * N + (bColBase + localCol)] if in-bounds else 0.0f
        //       store into sB[localRow * sB_row_stride + localCol]

        __syncthreads();

        // --- Compute partial sums ---
        // TODO: For k in [0, TILE):
        //         b_k = sB[k * sB_row_stride + localCol]
        //         For e in [0, elems_per_thread):
        //           sA_row = localRow * elems_per_thread + e
        //           a_k = sA[sA_row * sA_row_stride + k]
        //           acc[e] += a_k * b_k
    ...
    // TODO: For e in [0, elems_per_thread):
    //         grow = threadRowStart + e
    //         if (grow < N && globalCol < N): C[grow * N + globalCol] = acc[e]
}
