    ...
    for (int t = 0; t < numTiles; t++) {
        ...
        for (int i = 0; i < R; i++) {
            ...
            for (int j = 0; j < R; j++) {
                ...
                if (r < N && cc < N)
                    sA[sRow * TILE + sCol] = A[r * N + cc];
                else
                    sA[sRow * TILE + sCol] = 0.0f;
            }
        }
        ...
        for (int i = 0; i < R; i++) {
            ...
            for (int j = 0; j < R; j++) {
                ...
                if (r < N && cc < N)
                    sB[sRow * TILE + sCol] = B[r * N + cc];
                else
                    sB[sRow * TILE + sCol] = 0.0f;
            }
        }
        ...
        for (int k = 0; k < TILE; k++) {
            for (int i = 0; i < R; i++) {
                float aval = sA[(ty * R + i) * TILE + k];
                for (int j = 0; j < R; j++) {
                    float bval = sB[k * TILE + (tx * R + j)];
                    reg[i][j] += aval * bval;
                }
            }
        }
